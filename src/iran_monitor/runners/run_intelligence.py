...
def collect_rss(config, limit: int) -> list[NewsItem]:
    items: list[NewsItem] = []
    for source in config.rss:
        if source.enabled:
            items.extend(RSSCollector(source).collect()[:limit])
    return items
...
def main() -> None:
    ...
    if args.publish:
        TelegramPublisher().publish(feed.report, feed.map_path, feed.assessment_path, feed.pie_path)
